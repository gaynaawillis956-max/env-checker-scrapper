			...
			try:
				...
				while True:
					...
					try:
						...
						mail_to = extract_mail(mail_str)
						if time.perf_counter() - server_born_time < 60*1:
							chill_time = 6
							msg = cyan('+\b'+'>'*(mails_sent%3)+b+'>',0)+cyan('>> '[mails_sent%3:],0)+'warming-up, sleeping '+bold('6s')
							...
							chill_time = 0.1
							msg = cyan('+\b'+'>'*(mails_sent%3)+b+'>',0)+cyan('>> '[mails_sent%3:],0)+'chilling for 0.1s between emails'
...
def load_config():
	global config, smtp_pool_array, threads_count, connection_timeout
	...
	if not is_file_or_url(config['smtps_list_file']):
		...
		smtp_pool_array = [l for l in read_lines(config['smtps_list_file']) if l.strip()]
	...
	# Apply speed settings before cap so the cap uses the intended thread count
	if config.get('threads_count', '').strip():
		try: threads_count = max(1, int(config['threads_count']))
		except: pass
	if config.get('connection_timeout', '').strip():
		try: connection_timeout = max(1, int(config['connection_timeout']))
		except: pass
...
slow_send_mail_servers_delay = 3
total_sent = 0
skipped = 0
speed = []
mem_usage = 0
